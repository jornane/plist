...
public class NSUserByHostDefaults extends NSDefaults {
	private static final long serialVersionUID = 1L;
	
	/** All instances */
	protected final static HashMap<String,NSUserByHostDefaults> INSTANCES = new HashMap<String,NSUserByHostDefaults>();
	
	/** This defaults' scope */
	public static final Scope SCOPE = Scope.USER_BYHOST;
	
	/**
	 * Construct new defaults instance for a given domain
	 * @param domain	the domain
	 */
	private NSUserByHostDefaults(String domain) {
		super(OSPATH.getPListFile(domain, SCOPE));
	}
	
	/** @see NSDefaults#getInstance(String, Scope) */
	public synchronized static final NSUserByHostDefaults getInstance(String domain) {
		if (INSTANCES.containsValue(domain))
			return INSTANCES.get(domain);
		NSUserByHostDefaults result = new NSUserByHostDefaults(domain);
