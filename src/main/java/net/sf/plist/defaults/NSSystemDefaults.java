...
public class NSSystemDefaults extends NSDefaults {
	private static final long serialVersionUID = 1L;
	
	/** All instances */
	protected final static HashMap<String,NSSystemDefaults> INSTANCES = new HashMap<String,NSSystemDefaults>();
	
	/** This defaults' scope */
	public static final Scope SCOPE = Scope.SYSTEM;
	
	/**
	 * Construct new defaults instance for a given domain
	 * @param domain	the domain
	 */
	private NSSystemDefaults(String domain) {
		super(OSPATH.getPListFile(domain, SCOPE));
	}
	
	/** @see NSDefaults#getInstance(String, Scope) */
	public synchronized static final NSSystemDefaults getInstance(String domain) {
		if (INSTANCES.containsValue(domain))
			return INSTANCES.get(domain);
		NSSystemDefaults result = new NSSystemDefaults(domain);
